...
// --- GERAR 2ª VIA (ATUALIZADA COM ASSINATURAS DO BANCO) ---
function gerar2ViaPublica() {
    ...
    // RECUPERAR ASSINATURAS DO OBJETO DADOSCONSULTA
    let blocoAssinaturaFamilia = "";
    if (d.assinatura_responsavel) {
        blocoAssinaturaFamilia = `<div style="text-align:center;"><img src="${d.assinatura_responsavel}" style="max-height:60px; max-width:80%;"></div>`;
    } else {
    ...
    if (d.assinatura_atendente) {
        blocoAssinaturaAtendente = `<div style="text-align:center;"><img src="${d.assinatura_atendente}" style="max-height:60px; max-width:80%;"></div>`;
    } else {
    ...
    <div class="obs-box">OBS: PASSANDO DAS 36 HORAS DO FALECIMENTO SOMENTE COM TANATOPRAXIA.</div><div class="obs-box">OBS.: VELÓRIO COM DURAÇÃO DE DUAS HORAS ANTES DO SEPULTAMENTO. EM CASO DE ATRASO DO SERVIÇO FUNERÁRIO NÃO SERÁ ESTENDIDO O HORÁRIO ESTABELECIDO.</div>
    <div class="line" style="margin-top: 15px; border: 2px solid #000; padding: 5px;"><span class="bold">PREVISÃO DE EXUMAÇÃO:</span> A partir de <span class="red" style="font-size:16px;">${dataExumacao}</span><br><span style="font-size:10px;">(Legislação: 3 anos para Adultos / 2 anos para Crianças até 11 anos)</span>
