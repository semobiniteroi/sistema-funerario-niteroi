...
function liberarAcesso() {
    ...
    // Atualiza tabela com data local correta
...
function renderizarTabela(lista) {
    ...
    if (lista.length === 0) {
        tbody.innerHTML = '<tr><td colspan="11" style="padding: 40px; text-align:center; color:#b5b5c3;">Nenhum atendimento registrado.</td></tr>';
        return;
    }
...
window.visualizar = function(id) {
    ...
    db.collection("atendimentos").doc(id).get().then((doc) => {
        ...
        if (doc.exists) {
            ...
            let servicosView = []; 
            if (i.tanato === 'SIM') servicosView.push('Tanatopraxia'); 
            if (i.invol === 'SIM') servicosView.push('Invol'); 
            if (i.translado === 'SIM') servicosView.push('Translado'); 
            if (i.urna_opc === 'SIM') servicosView.push('Urna'); 
            ...
            let tipo = ''; if (i.gav && i.gav.includes('X')) tipo = 'GAVETA'; else if (i.car && i.car.includes('X')) tipo = 'CARNEIRO'; else if (i.cova_rasa === 'X') tipo = 'COVA RASA'; else if (i.perpetua === 'X') tipo = 'PERPÉTUA'; 
            
            if(document.getElementById('view_local_sepul')) {
                 document.getElementById('view_local_sepul').innerText = `Tipo: ${tipo} | Nº: ${i.sepul||'-'} | QD: ${i.qd||'-'}`;
            }

            setText('view_hospital_completo', i.hospital); 
            setText('view_cap', i.cap);
            ...
            if (i.geo_coords && i.geo_coords.includes(',')) { 
                mapContainer.style.display = 'block'; 
                mapFrame.innerHTML = `<iframe width="100%" height="100%" frameborder="0" style="border:0" src="https://maps.google.com/maps?q=${i.geo_coords}&z=17&output=embed"></iframe>`; 
                mapLink.href = `https://www.google.com/maps/search/?api=1&query=${i.geo_coords}`; 
            } else { 
                mapContainer.style.display = 'none'; mapFrame.innerHTML = ''; 
            }
...
window.editar = function(id) {
    db.collection("atendimentos").doc(id).get().then((doc) => {
        if (doc.exists) {
            ...
            const selectHora = document.getElementById('hora'); 
            if(![...selectHora.options].some(o => o.value == item.hora)) {
                const opt = document.createElement('option'); opt.value = item.hora; opt.text = item.hora; selectHora.add(opt); 
            }
            selectHora.value = item.hora || "";
            ...
            setVal('atendente_sistema', item.atendente_sistema);
            ...
            setVal('sepul', item.sepul); setVal('motivo_troca_sepultura', item.motivo_troca_sepultura);
            setVal('qd', item.qd); setVal('hospital', item.hospital); setVal('estado_obito', item.estado_obito);
            
            if (item.estado_obito) { carregarCidades(item.estado_obito, item.cidade_obito); } 
            else { document.getElementById('cidade_obito').innerHTML = '<option value="">Selecione a UF primeiro</option>'; document.getElementById('cidade_obito').disabled = true; }
...
form.onsubmit = (e) => {
    ...
    const dados = {
        ...
        protocolo: document.getElementById('protocolo_hidden').value
    };

    if(!id && !dados.protocolo) { dados.protocolo = gerarProtocolo(); }

    if (id) { 
        db.collection("atendimentos").doc(id).update(dados).then(() => window.fecharModal()).catch((error) => alert("Erro ao atualizar: " + error)); 
    } else { 
        db.collection("atendimentos").add(dados).then(() => window.fecharModal()).catch((error) => alert("Erro ao salvar: " + error)); 
    }
};

window.excluir = function(id) { 
    if(confirm('Tem certeza?')) { db.collection("atendimentos").doc(id).delete().catch((error) => alert("Erro ao excluir: " + error)); } 
}

// -- CORREÇÃO DO FECHAMENTO AO CLICAR FORA --
window.onclick = function(event) { 
    const t = event.target;
    // O modal principal NÃO fecha mais clicando fora (apenas botões)
...
window.imprimirRelatorio = function(modo) { 
    const oldStyle = document.getElementById('print-style'); if (oldStyle) oldStyle.remove();
    const style = document.createElement('style'); style.id = 'print-style'; style.innerHTML = `@page { size: ${modo}; margin: 5mm; }`; document.head.appendChild(style); 
    ...
    fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${uf}/municipios`)
        ...
        .then(cidades => { 
            ...
            cidades.forEach(cidade => { 
                const opt = document.createElement('option'); opt.value = cidade.nome.toUpperCase(); opt.text = cidade.nome.toUpperCase(); 
                if (cidade.nome.toUpperCase() === cidadeSelecionada) opt.selected = true; 
                selectCidade.appendChild(opt); 
            }); 
            selectCidade.disabled = false; 
        })
        .catch(err => { console.error(err); selectCidade.innerHTML = '<option value="">Erro ao carregar</option>'; }); 
}
...
// FUNÇÃO GPS
window.pegarLocalizacaoGPS = function() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            const lat = position.coords.latitude;
            const long = position.coords.longitude;
            document.getElementById('geo_coords').value = `${lat}, ${long}`;
...
// --- ETIQUETA OTIMIZADA PARA A4 PAISAGEM (1 FOLHA) ---
window.gerarEtiqueta = function() {
    ...
    <head>
        ...
        <style>
            ...
            .title { font-size: 24px; font-weight: 900; text-transform: uppercase; margin-bottom: 5px; border-bottom: 3px solid #000; display: inline-block; padding-bottom: 5px; }
            
            .label { font-size: 20px; color: #333; font-weight: bold; text-transform: uppercase; margin-bottom: 2px; }
            
            .val-nome { font-size: 60px; font-weight: 900; text-transform: uppercase; line-height: 1.1; margin-bottom: 15px; }
            
            .val-data { font-size: 35px; font-weight: 800; margin-bottom: 15px; }
            
            .val-local { font-size: 30px; font-weight: 800; text-transform: uppercase; }
        </style>
    </head>
    <body>
        <div class="box">
            <div>
                <img src="https://niteroi.rj.gov.br/wp-content/uploads/2025/06/pmnlogo-2.png" class="header-img"><br>
...
window.enviarWhatsapp = function() {
    if (!dadosAtendimentoAtual) { alert("Selecione um atendimento."); return; }
    ...
    if (!coords) { alert("Sem GPS cadastrado."); return; }
...
window.enviarSMS = function() {
    ...
    const coords = dadosAtendimentoAtual.geo_coords;
    if (!tel) { alert("Sem telefone cadastrado."); return; }
    const msg = `Localização sepultura: https://www.google.com/maps/search/?api=1&query=${coords}`;
    window.location.href = `sms:${tel}?body=${encodeURIComponent(msg)}`;
}

// --- COMPROVANTE RESTAURADO E OTIMIZADO ---
window.gerarComprovante = function() {
    ...
    <html><head><title>Comprovante</title><style>@page{size:A4 portrait;margin:15mm}body{font-family:Arial,sans-serif;font-size:11px;margin:0;padding:10px;line-height:1.4}.header{text-align:center;margin-bottom:20px}.header h2{font-size:14px;text-decoration:underline;margin:0;font-weight:bold;text-transform:uppercase}.protocolo{position:absolute;top:10px;right:10px;font-size:12px;font-weight:bold;border:1px solid #000;padding:5px}.content{width:100%}.line{margin-bottom:8px}.bold{font-weight:bold}.section-title{font-weight:bold;margin-top:15px;margin-bottom:5px;text-transform:uppercase}.box-lateral{float:right;width:220px;border:1px solid #000;padding:5px;font-size:10px;font-weight:bold;margin-left:10px;margin-bottom:10px;text-align:left}.termo-texto{text-align:justify;margin-right:240px}.assinaturas{margin-top:40px;display:flex;justify-content:space-between;gap:20px}.assinatura-linha{border-top:1px solid #000;flex:1;text-align:center;padding-top:5px}.obs-box{margin-top:15px;font-weight:bold}.footer-info{margin-top:30px;border-top:1px solid #000;pt:10px}.aviso-final{margin-top:20px;font-weight:bold;text-align:justify;border:1px solid #000;padding:5px}</style></head><body>
        <div class="protocolo">PROTOCOLO: ${protocolo}</div><div class="header"><h2>Comprovante de Atendimento</h2></div><div class="content">
            ...
            <div class="line"><span class="bold">COM CAPELA</span> ${chk(comCapela)} <span class="bold">SEM CAPELA</span> ${chk(!comCapela)} <span class="bold" style="margin-left:20px">DATA DO FALECIMENTO:</span> ${fmtData(d.data_obito)} AS ${d.hora_obito} <span class="bold" style="margin-left:10px; color:red;">[${tempoDecorrido}]</span></div>
