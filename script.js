...
window.copiarNomePasta = function() { const texto = document.getElementById('onedrive_nome_pasta').innerText; navigator.clipboard.writeText(texto).then(() => { alert("Nome copiado! Vá para o OneDrive, crie a pasta e cole este nome."); }).catch(err => { alert("Erro ao copiar automaticamente. Selecione o texto e copie manualmente."); }); }
...
window.gerarComprovante = function() {
    ...
    let txtSep = (d.tipo_sepultura || "").toUpperCase(), co = d.classificacao_obito || "ADULTO", txtHO = d.hora_obito || ""; if (d.ignorar_hora_obito === 'SIM') txtHO += " (IGNORADO)"; let c = co === "ANJO" ? "ANJO" : "ADULTO";
...
// --- DOM INIT, FIX DATA E VISUALIZAR ---
document.addEventListener('DOMContentLoaded', () => {
    ...
    // AUTO-FIX: Corrige registros que foram salvos com a chave de data errada (invisiveis)
    setTimeout(() => {
        if(getDB()) {
            getDB().collection("atendimentos").orderBy("data_hora_atendimento", "desc").limit(50).get().then(snap => {
                snap.forEach(doc => { const d = doc.data(); if(d.data_ficha_modal && !d.data_ficha) { getDB().collection("atendimentos").doc(doc.id).update({ data_ficha: d.data_ficha_modal }); } });
            });
        }
    }, 3000);
});
