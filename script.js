...
window.toggleIndigente = function() {
    ...
    const camposObrigatorios = ['resp_nome', 'telefone', 'funeraria', 'isencao', 'tipo_sepultura', 'sepul', 'qd', 'hospital', 'cap', 'data_obito', 'nome', 'causa', 'hora'];

    camposObrigatorios.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            if (chk && chk.checked) {
                // Se marcado, remove required
                if (el.hasAttribute('required')) {
                    el.removeAttribute('required');
                    el.setAttribute('data-was-required', 'true');
                }
            } else {
                // Se desmarcado, restaura required se ele tinha antes
...
window.salvarAssinatura = function() {
    ...
    if (canvas) {
        ...
        // Verifica se temos um atendimento carregado com ID
        if (dadosAtendimentoAtual && dadosAtendimentoAtual.id) {
            let updateData = {};
            
            if (tipoAssinaturaAtual === 'responsavel') {
                assinaturaResponsavelImg = imgData; // Atualiza variável local
                dadosAtendimentoAtual.assinatura_responsavel = imgData; // Atualiza objeto em memória
                updateData = { assinatura_responsavel: imgData };
            } else {
                assinaturaAtendenteImg = imgData; // Atualiza variável local
                dadosAtendimentoAtual.assinatura_atendente = imgData; // Atualiza objeto em memória
                updateData = { assinatura_atendente: imgData };
            }

            // Atualiza no Firestore sem alert para não travar
...
window.renderizarTabela = function(lista) {
    ...
    lista.forEach(item => {
        ...
        if (item.perpetua === 'X' || tipoSep.includes('PERPETU')) {
             ...
             labelPerpetua = `<div style="font-weight:bold; font-size:10px; color:#2196F3; margin-top:2px;">${labelTexto}</div><div style="font-weight:bold; font-size:10px; color:#2196F3;">L: ${item.livro_perpetua||''} F: ${item.folha_perpetua||''}</div>`;
        }
...
window.abrirModal = function() {
    ...
    document.getElementById('docId').value = ""; document.getElementById('protocolo_hidden').value = "";
    document.getElementById('div-motivo-edicao').classList.add('hidden');
    
    // CORREÇÃO: Preencher Atendente
...
window.editar = function(id) {
    ...
    database.collection("atendimentos").doc(id).get().then(doc => {
        if(doc.exists) {
            ...
            // RESETAR ASSINATURAS AO ABRIR (LOAD FROM DB IF EXISTS)
...
if(form) {
    form.onsubmit = (e) => {
        ...
        // Garantir que o nome do atendente vai
        ...
        if(id) {
            ...
            database.collection("auditoria").add({ data_log: new Date().toISOString(), usuario: usuarioLogado ? usuarioLogado.nome : 'Anon', acao: "EDIÇÃO", detalhe: `ID: ${id} | Motivo: ${motivo}` });
            database.collection("atendimentos").doc(id).update(dados).then(() => window.fecharModal());
        } else {
             database.collection("auditoria").add({ data_log: new Date().toISOString(), usuario: usuarioLogado ? usuarioLogado.nome : 'Anon', acao: "CRIAÇÃO", detalhe: `Novo: ${dados.nome}` });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(chkIndigente) { chkIndigente.addEventListener('change', window.toggleIndigente); }

    const seletorCausas = document.getElementById('seletor_causas');
    const inputCausa = document.getElementById('causa');
    if (seletorCausas && inputCausa) { seletorCausas.addEventListener('change', function() { if (this.value) { if (inputCausa.value === "") { inputCausa.value = this.value; } else { inputCausa.value += " / " + this.value; } this.value = ""; } }); }
    const selectTipo = document.getElementById('tipo_sepultura');
    if (selectTipo) { selectTipo.addEventListener('change', function() { const val = this.value.toUpperCase(); const divP = document.getElementById('div-perpetua'); if (val.includes('PERPETUA') || val.includes('PERPETUO')) { divP.classList.remove('hidden'); } else { divP.classList.add('hidden'); } }); }
    const fl = document.getElementById('filtro-local');
    if(fl) fl.addEventListener('change', (e) => { window.atualizarListener(document.getElementById('filtro-data').value, e.target.value); window.atualizarLabelQuadra(e.target.value); });
    const sessao = sessionStorage.getItem('usuarioLogado');
    if (sessao) { usuarioLogado = JSON.parse(sessao); safeDisplay('tela-bloqueio', 'none'); if(document.getElementById('filtro-local')) window.atualizarListener(window.pegarDataAtualLocal(), document.getElementById('filtro-local').value); }
    const fd = document.getElementById('filtro-data');
...
window.visualizar = function(id) {
    ...
    database.collection("atendimentos").doc(id).get().then(doc => {
        if(doc.exists) {
            const d = doc.data();
            d.id = doc.id; // GARANTE QUE O ID SEJA SALVO NO OBJETO
            ...
            if(d.tipo_sepultura && d.tipo_sepultura.toUpperCase().includes('PERPETU')) { localSep += ` (L: ${d.livro_perpetua||'-'} F: ${d.folha_perpetua||'-'})`; }
            ...
            let dataHoraFinal = "";
            if (d.data_hora_atendimento) { const date = new Date(d.data_hora_atendimento); dataHoraFinal = `${date.getDate().toString().padStart(2,'0')}/${(date.getMonth()+1).toString().padStart(2,'0')}/${date.getFullYear()} AS ${date.getHours().toString().padStart(2,'0')}:${date.getMinutes().toString().padStart(2,'0')}`; } 
            else if (d.protocolo && d.protocolo.length >= 13) { const p = d.protocolo; if(p.indexOf('-') === 8) { dataHoraFinal = `${p.substring(6,8)}/${p.substring(4,6)}/${p.substring(0,4)} AS ${p.substring(9,11)}:${p.substring(11,13)}`; } }
            ...
            if (mapContainer && mapFrame && mapLink) { const cleanCoords = d.geo_coords ? d.geo_coords.replace(/\s/g, '') : ''; if (cleanCoords && cleanCoords.includes(',')) { mapContainer.style.display = 'block'; mapFrame.innerHTML = `<iframe width="100%" height="100%" frameborder="0" style="border:0" src="https://maps.google.com/maps?q=${cleanCoords}&z=17&output=embed"></iframe>`; mapLink.href = `https://www.google.com/maps?q=${cleanCoords}`; } else { mapContainer.style.display = 'none'; } }
...
window.imprimirRelatorio = function(modo) { const style = document.createElement('style'); style.innerHTML = `@page { size: ${modo}; }`; document.head.appendChild(style); window.print(); setTimeout(() => document.head.removeChild(style), 1000); };
...
// --- GERAR COMPROVANTE (ATUALIZADO COM 2 ASSINATURAS E NOME ATENDENTE) ---
window.gerarComprovante = function() {
    ...
    } else { const now = new Date(); dataHoraAtendimentoTexto = `${now.getDate().toString().padStart(2,'0')}/${(now.getMonth()+1).toString().padStart(2,'0')}/${now.getFullYear()} AS ${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}`; }
    ...
    let tempoDecorrido = ""; if (d.data_obito && d.hora_obito && d.hora && d.data_ficha) { const dtObito = new Date(d.data_obito + 'T' + d.hora_obito); const dtSepul = new Date(d.data_ficha + 'T' + d.hora); const diff = dtSepul - dtObito; if (diff > 0) { const diffHrs = Math.floor(diff / 3600000); const diffMins = Math.round(((diff % 3600000) / 60000)); tempoDecorrido = `${diffHrs}h ${diffMins}min`; } }
    const ec = d.estado_civil || ""; const chkEC = (val) => ec === val ? '(X)' : '( )'; const relacao = d.parentesco ? `(${d.parentesco})` : '';
    let txtSep = (d.tipo_sepultura || "").toUpperCase(); const co = d.classificacao_obito || "ADULTO"; let txtHoraObito = d.hora_obito; if (d.ignorar_hora_obito === 'SIM') txtHoraObito += " (IGNORADO)"; let classificacao = co; if (txtSep.includes("ANJO")) classificacao = "ANJO";
    ...
    <div class="obs-box">OBS: PASSANDO DAS 36 HORAS DO FALECIMENTO SOMENTE COM TANATOPRAXIA.</div><div class="obs-box">OBS.: VELÓRIO COM DURAÇÃO DE DUAS HORAS ANTES DO SEPULTAMENTO. EM CASO DE ATRASO DO SERVIÇO FUNERÁRIO NÃO SERÁ ESTENDIDO O HORÁRIO ESTABELECIDO.</div><div class="line" style="margin-top: 15px; border: 2px solid #000; padding: 5px;"><span class="bold">PREVISÃO DE EXUMAÇÃO:</span> A partir de <span class="red" style="font-size:16px;">${dataExumacao}</span><br><span style="font-size:10px;">(Legislação: 3 anos para Adultos / 2 anos para Crianças até 11 anos)</span>
    ...
    </div><div class="two-columns"><div class="col-left"><div style="text-align:center; font-weight:bold; text-decoration:underline; margin-bottom:5px;">TERMO DE COMPROMISSO CEMITÉRIOS MUNICIPAIS</div><div class="termo-juridico">Sendo o <span class="bold">FALECIDO CASADO</span>, o responsável perante, o Cemitério do MARUÍ, SÃO FRANCISCO E ITAIPU será obrigatoriamente o <span class="bold">CONJUGE</span>.<br>Sendo o <span class="bold">FALECIDO VIÚVO</span>, os responsáveis perante o CEMITÉRIO do MARUÍ, SÃO FRANCISCO E ITAIPU serão obrigatoriamente os <span class="bold">FILHOS</span>.<br>Sendo o <span class="bold">FALECIDO SOLTEIRO</span>, os responsáveis perante o CEMITÉRIO do MARUÍ, SÃO FRANCISCO E ITAIPU obrigatoriamente os <span class="bold">FILHOS, PAIS, IRMÃOS</span>.<br>Será exigida a apresentação de documentos de <span class="bold" style="text-decoration:underline">IDENTIDADE e CPF</span>.</div>
