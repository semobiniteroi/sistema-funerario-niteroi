...
// Variáveis para Estatísticas e Exportação
...
window.carregarEstatisticas = function(dias) {
    ...
    statsUnsubscribe = db.collection("atendimentos")
        ...
        .onSnapshot((snapshot) => {
            ...
            // Preparar dados para exportação e gráfico
            const ranking = Object.entries(contagemCausas).sort((a, b) => b[1] - a[1]);
            
            // Salvar para exportação global
...
function renderizarGrafico(labels, dataValues) {
    ...
    chartInstance = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels.slice(0, 10), // Top 10 para não poluir
            datasets: [{
                label: 'Ocorrências',
        ...
        options: {
            ...
            plugins: {
                ...
                title: { display: true, text: 'Top 10 Causas de Morte' }
            }
        }
    });
}
...
window.baixarExcel = function() {
    if (dadosEstatisticasExportacao.length === 0) {
...
// (Mantive todas as funções anteriores intactas abaixo)
...
window.gerarComprovante = function() {
    ...
    <html><head><title>Comprovante</title><style>
        ...
        <div class="box">
            ...
            <span class="linha"><span class="campo">CEMITÉRIO:</span> ${chk(d.local.includes('MARUÍ'))} MARUÍ ${chk(d.local.includes('SÃO FRANCISCO'))} SÃO FRANCISCO ${chk(d.local.includes('ITAIPU'))} ITAIPU</span>
            <span class="linha"><span class="campo">SEPULTURA:</span> ${d.sepul} <span class="campo">QUADRA:</span> ${d.qd} <span class="campo">CAPELA:</span> ${d.cap}</span>
            <span class="linha"><span class="campo">DATA FALECIMENTO:</span> ${fmtData(d.data_obito)} <span class="campo">HORA:</span> ${d.hora_obito}</span>
        </div>
        <div class="box">
            ...
            <div class="linha">${chk(d.tipo_sepultura === 'GAVETA' && d.classificacao_obito === 'ADULTO')} Gaveta Adulto ${chk(d.tipo_sepultura === 'CARNEIRO' && d.classificacao_obito === 'ADULTO')} Carneira Adulto ${chk(d.tipo_sepultura === 'COVA RASA' && d.classificacao_obito === 'ADULTO')} Cova Rasa Adulto</div>
            <div class="linha">${chk(d.tipo_sepultura === 'GAVETA' && d.classificacao_obito === 'ANJO')} Gaveta Anjo ${chk(d.tipo_sepultura === 'CARNEIRO' && d.classificacao_obito === 'ANJO')} Carneira Anjo ${chk(d.tipo_sepultura === 'PERPETUA')} Perpétuo</div>
            <div class="linha" style="margin-top: 10px;"><span class="campo">TANATOPRAXIA:</span> ${chk(d.tanato === 'SIM')} SIM ${chk(d.tanato !== 'SIM')} NÃO</div>
        </div>
        <div class="obs-box"><strong>OBSERVAÇÕES:</strong><br>1. PASSANDO DAS 36 HORAS DO FALECIMENTO SOMENTE COM TANATOPRAXIA.<br>2. VELÓRIO COM DURAÇÃO DE DUAS HORAS ANTES DO SEPULTAMENTO.<br>3. EM CASO DE ATRASO DO SERVIÇO FUNERÁRIO NÃO SERÁ ESTENDIDO O HORÁRIO ESTABELECIDO.<br>4. TAXAS MUNICIPAIS E INVOL DEVEM SER PAGOS COM DUAS HORAS DE ANTECEDÊNCIA.</div>
        <div class="assinaturas"><div class="assinatura-box">Assinatura Funcionário (Acolhimento)</div><div class="assinatura-box">Assinatura Responsável / Família<br><span style="font-size:10px;">${d.resp_nome.toUpperCase()}</span></div></div>
...
function carregarCidades(uf, cidadeSelecionada = "") {
    ...
    fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${uf}/municipios`).then(r => r.json()).then(c => {
        ...
        c.forEach(cid => {
            const opt = document.createElement('option'); opt.value = cid.nome.toUpperCase(); opt.text = cid.nome.toUpperCase();
            if (cid.nome.toUpperCase() === cidadeSelecionada) opt.selected = true;
