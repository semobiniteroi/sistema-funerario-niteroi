...
// ============================================================================
const SYSTEM_VERSION = "1.5"; // Variável que controla a exibição da tela de novidades
...
// ============================================================================
// IMPORTAÇÃO E BUSCA DE BENEFÍCIOS (PORTAL DA TRANSPARÊNCIA E API)
// ============================================================================
window.importarBeneficiarios = function() {
    ...
    reader.onload = async function(e) {
        try {
            ...
            if (rows.length === 0) { 
                statusEl.innerText = "A planilha parece estar vazia."; 
                return; 
            }
            ...
            filtrados.forEach(d => {
                ...
                html += `
                    <div style="background:#fff; border:1px solid #cbd5e1; padding:10px; border-radius:6px; display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <button type="button" class="btn-novo" style="background-color:#10b981; padding:4px 8px; font-size:11px; width:auto;" onclick="aplicarIsencaoBeneficio('${safeName}', '${d.cpf_parcial || ''}', '${d.nis || ''}')">Aplicar Isenção</button>
                    </div>
                `;
            });
            html += '</div>';
...
window.consultarApiGoverno = async function() {
    ...
    let cpfBusca = document.getElementById('input_busca_beneficio').value.replace(/\D/g, '');
    const divRes = document.getElementById('resultado-busca-beneficio');
    ...
    try {
        ...
        // Fallback: usa a chave fornecida na documentação caso não tenha sido salva no painel
        ...
        for (let anoMes of mesesParaTestar) {
            let urlBolsa = proxyUrl + encodeURIComponent(`https://api.portaldatransparencia.gov.br/api-de-dados/bolsa-familia-disponivel-por-cpf-ou-nis?codigo=${cpfBusca}&anoMesCompetencia=${anoMes}&pagina=1`);
            let urlBpc = proxyUrl + encodeURIComponent(`https://api.portaldatransparencia.gov.br/api-de-dados/bpc-por-cpf-ou-nis?codigo=${cpfBusca}&anoMesCompetencia=${anoMes}&pagina=1`);
            
            let resBolsa = await fetch(urlBolsa, { method: 'GET', headers: headers }).catch(() => null);
            let resBpc = await fetch(urlBpc, { method: 'GET', headers: headers }).catch(() => null);
        ...
        if (beneficiosEncontrados.length > 0) {
            ...
            divRes.innerHTML = `
                <div style="background:#ecfdf5; border:1px solid #10b981; padding:15px; border-radius:6px; text-align:center;">
                    ...
                    <button type="button" class="btn-novo" style="background-color:#10b981; padding:8px 15px; font-size:12px; width:100%;" onclick="aplicarIsencaoBeneficio('${benefText.replace(/'/g, "\\'")}', '${cpfBusca}', '')">✅ Aplicar Isenção Oficial</button>
                </div>
            `;
        } else {
            divRes.innerHTML = `<span style="color:#ef4444; font-size:12px;">❌ Nenhum benefício ativo encontrado para este número na API Federal nos últimos 3 meses.<br>A API pode ter atrasos de atualização ou estar bloqueando por CORS. Tente a busca Local.</span>`;
        }
    } catch (error) { 
        divRes.innerHTML = '<span style="color:#ef4444; font-size:12px;">❌ Erro ao conectar com o Governo (Verifique sua conexão ou CORS). Tente usar a busca Local.</span>'; 
        console.error(error);
    }
            ...
            snap.forEach(doc => {
                ...
                html += `
                    <div class="historico-item">
                        <div class="historico-icon" style="background:#f1f5f9;color:${cor};">${icone}</div>
...
window.fazerLogin = async function() {
    ...
    try {
        ...
        if(msgErro) {
            let primeiroNome = usuarioLogado.nome ? usuarioLogado.nome.split(' ')[0] : 'Usuário';
            msgErro.innerText = `Login com sucesso! Bem-vindo(a), ${primeiroNome}.`;
            msgErro.style.color = "#10b981";
            msgErro.style.display = 'block';
        }
...
window.liberarAcesso = function() { 
    ...
    // Injetar o nome do usuário no Header
    const elUserDisplay = document.getElementById('user-display');
    if (elUserDisplay && usuarioLogado && usuarioLogado.nome) {
        let primeiroNome = usuarioLogado.nome.split(' ')[0];
        elUserDisplay.innerHTML = `<span style="font-size: 13px; color: #3b82f6; margin-right: 15px; font-weight: 600;">👋 Olá, ${primeiroNome}</span>`;
    }
    
    if(getDB()) {
        getDB().collection("config").doc("geral").get().then(doc => {
            if(doc.exists && doc.data().wpp_fixo) { 
                numeroWppFixo = doc.data().wpp_fixo; 
            }
        }).catch(e => console.log("Aviso: Configurações de WhatsApp não carregadas."));
    }
    ...
    // POPUP DE NOVIDADES DO SISTEMA APÓS O LOGIN
    setTimeout(() => {
...
// ============================================================================
// RENDERIZAÇÃO DE TABELAS E CARDS
// ============================================================================
...
window.abrirAba = function(id) {
    ...
    if(id === 'tab-config') {
        getDB().collection("config").doc("geral").get().then(doc => {
            if(doc.exists) {
                if(document.getElementById('config_wpp_fixo')) document.getElementById('config_wpp_fixo').value = doc.data().wpp_fixo || '';
                if(document.getElementById('config_api_transparencia')) document.getElementById('config_api_transparencia').value = doc.data().api_transparencia || '';
            }
        });
    }
}
...
window.salvarConfiguracoesGerais = function() {
    const num = document.getElementById('config_wpp_fixo') ? document.getElementById('config_wpp_fixo').value.replace(/\D/g, '') : numeroWppFixo;
    const api = document.getElementById('config_api_transparencia') ? document.getElementById('config_api_transparencia').value.trim() : '';
...
window.gerarComprovante = function() {
    ...
    const co = d.classificacao_obito || "ADULTO"; 
    let tH = d.hora_obito || ""; 
    if(d.ignorar_hora_obito === 'SIM' || d.chk_ignorar_hora === 'SIM') tH += " (IGNORADO)";
...
window.onclick = function(e) { 
    const m = ['modal-visualizar', 'modal-estatisticas', 'modal-admin', 'modal-transferir', 'modal-whatsapp', 'modal-agencia', 'modal-liberacao', 'modal-transferir-responsavel', 'modal-onedrive', 'modal-docs-acolhimento', 'modal-particular', 'modal-assinatura', 'modal-particular-ficha', 'modal-historico', 'modal-busca-beneficio', 'modal-novidades']; 
