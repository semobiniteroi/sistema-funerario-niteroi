function fazerLogin() {
    ...
    const database = getDB(); if (!database) { alert("Sem conexão com banco."); return; }
...
let db = null, unsubscribe = null, statsUnsubscribe = null, equipeUnsubscribe = null, logsUnsubscribe = null, sepulturaOriginal = "", dadosAtendimentoAtual = null, dadosEstatisticasExportacao = [], chartInstances = {}, usuarioLogado = null, signaturePad = null, isDrawing = false, assinaturaResponsavelImg = null, assinaturaAtendenteImg = null, tipoAssinaturaAtual = '', dashboardAtual = 'acolhimento'; window.idLiberacaoAtual = null; window.idTransferenciaResponsavelAtual = null; window.nomePastaPadrao = 'PROTOCOLO_NOME';
...
window.alternarDashboard = function(dash) {
    dashboardAtual = dash; const btnAcolhimento = document.getElementById('nav-btn-acolhimento'), btnAgencia = document.getElementById('nav-btn-agencia'); const divAcolhimento = document.getElementById('dashboard-acolhimento'), divAgencia = document.getElementById('dashboard-agencia');
    if (dash === 'agencia') { btnAcolhimento.classList.remove('active'); btnAgencia.classList.add('active'); divAcolhimento.classList.add('hidden'); divAgencia.classList.remove('hidden'); } 
    else { btnAgencia.classList.remove('active'); btnAcolhimento.classList.add('active'); divAgencia.classList.add('hidden'); divAcolhimento.classList.remove('hidden'); }
...
window.buscarCEP = function(cep) {
    cep = cep.replace(/\D/g, ''); if (cep.length === 8) { fetch(`https://viacep.com.br/ws/${cep}/json/`).then(res => res.json()).then(data => { if (!data.erro) { document.getElementById('resp_endereco').value = data.logradouro.toUpperCase(); document.getElementById('resp_bairro').value = data.bairro.toUpperCase(); document.getElementById('resp_cidade').value = data.localidade.toUpperCase(); document.getElementById('resp_uf').value = data.uf.toUpperCase(); document.getElementById('resp_numero').focus(); } }).catch(err => console.error(err)); }
}
window.buscarPorCPF = function() {
    let cpfInput = document.getElementById('resp_cpf').value.replace(/\D/g, ''); if (!cpfInput) { alert("Digite um CPF válido."); return; }
    getDB().collection("atendimentos").where("resp_cpf", "==", cpfInput).limit(1).get().then(snap => {
        if (!snap.empty) { const d = snap.docs[0].data(); if(d.resp_nome) document.getElementById('resp_nome').value = d.resp_nome; if(d.resp_rg) document.getElementById('resp_rg').value = d.resp_rg; if(d.telefone) document.getElementById('telefone').value = d.telefone; if(d.resp_cep) document.getElementById('resp_cep').value = d.resp_cep; if(d.resp_endereco) document.getElementById('resp_endereco').value = d.resp_endereco; if(d.resp_numero) document.getElementById('resp_numero').value = d.resp_numero; if(d.resp_complemento) document.getElementById('resp_complemento').value = d.resp_complemento; if(d.resp_bairro) document.getElementById('resp_bairro').value = d.resp_bairro; if(d.resp_cidade) document.getElementById('resp_cidade').value = d.resp_cidade; if(d.resp_uf) document.getElementById('resp_uf').value = d.resp_uf; alert("Dados preenchidos!"); } else { alert("Nenhum cadastro prévio encontrado."); }
    });
}
window.toggleIndigente = function() {
    const chk = document.getElementById('chk_indigente'); const camposObrigatorios = ['resp_nome', 'resp_cpf', 'resp_endereco', 'resp_numero', 'resp_bairro', 'resp_cidade', 'telefone', 'funeraria', 'isencao', 'tipo_sepultura', 'sepul', 'qd', 'hospital', 'cap', 'data_obito', 'nome', 'causa', 'hora'];
    camposObrigatorios.forEach(id => { const el = document.getElementById(id); if (el) { if (chk && chk.checked) { if (el.hasAttribute('required')) { el.removeAttribute('required'); el.setAttribute('data-was-required', 'true'); } } else { if (el.getAttribute('data-was-required') === 'true') { el.setAttribute('required', ''); el.removeAttribute('data-was-required'); } } } });
}

function setupSignaturePad() {
    const canvas = document.getElementById('signature-pad'); if (!canvas) return; const ctx = canvas.getContext('2d'); ctx.lineWidth = 2; ctx.lineJoin = 'round'; ctx.lineCap = 'round'; ctx.strokeStyle = '#000';
    function getPos(canvas, evt) { const rect = canvas.getBoundingClientRect(); let clientX = evt.touches ? evt.touches[0].clientX : evt.clientX; let clientY = evt.touches ? evt.touches[0].clientY : evt.clientY; return { x: (clientX - rect.left) * (canvas.width / rect.width), y: (clientY - rect.top) * (canvas.height / rect.height) }; }
...
window.abrirModalAssinatura = function(tipo) { tipoAssinaturaAtual = tipo; const titulo = document.getElementById('titulo-assinatura'); if(titulo) titulo.innerText = (tipo === 'responsavel') ? 'Assinatura do Responsável' : 'Assinatura da Equipe'; safeDisplay('modal-assinatura', 'flex'); window.limparAssinatura(); setTimeout(setupSignaturePad, 200); }
...
window.salvarAssinatura = function() {
    ...
    if (canvas && dadosAtendimentoAtual && dadosAtendimentoAtual.id) { const imgData = canvas.toDataURL('image/png'); let updateData = {}; if (tipoAssinaturaAtual === 'responsavel') { assinaturaResponsavelImg = imgData; updateData = { assinatura_responsavel: imgData }; } else { assinaturaAtendenteImg = imgData; updateData = { assinatura_atendente: imgData }; } getDB().collection("atendimentos").doc(dadosAtendimentoAtual.id).update(updateData).then(() => window.fecharModalAssinatura()); }
}
window.abrirModalTransferir = function() { if(!dadosAtendimentoAtual) return; const select = document.getElementById('novo_cemiterio_transferencia'); if(select) select.value = dadosAtendimentoAtual.local || "CEMITÉRIO DO MARUÍ"; safeDisplay('modal-transferir', 'flex'); }
window.fecharModalTransferir = function() { safeDisplay('modal-transferir', 'none'); }
window.confirmarTransferencia = function() {
    if(!dadosAtendimentoAtual || !dadosAtendimentoAtual.id) return; const novoLocal = document.getElementById('novo_cemiterio_transferencia').value, localAntigo = dadosAtendimentoAtual.local || "CEMITÉRIO DO MARUÍ";
    if(novoLocal === localAntigo) { alert("O atendimento já está neste cemitério."); return; }
    if(confirm(`Confirmar transferência de ${localAntigo} para ${novoLocal}?`)) { getDB().collection("atendimentos").doc(dadosAtendimentoAtual.id).update({ local: novoLocal }).then(() => { getDB().collection("auditoria").add({ data_log: new Date().toISOString(), usuario: usuarioLogado ? usuarioLogado.nome : 'Anon', acao: "TRANSFERÊNCIA", detalhe: `ID: ${dadosAtendimentoAtual.id} | De: ${localAntigo} Para: ${novoLocal}` }); alert("Atendimento transferido com sucesso!"); window.fecharModalTransferir(); window.fecharModalVisualizar(); }); }
}
window.gerarProtocolo = function() { const elData = document.getElementById('data_ficha'); const agora = new Date(); let ano, mes, dia; if (elData && elData.value) { const p = elData.value.split('-'); ano = p[0]; mes = p[1]; dia = p[2]; } else { ano = agora.getFullYear(); mes = String(agora.getMonth()+1).padStart(2,'0'); dia = String(agora.getDate()).padStart(2,'0'); } return `${ano}${mes}${dia}-${String(agora.getHours()).padStart(2,'0')}${String(agora.getMinutes()).padStart(2,'0')}`; }
...
window.carregarCidades = function(uf) { const elCidade = document.getElementById('cidade_obito'); if(!uf) { if(elCidade) { elCidade.innerHTML = '<option value="">UF</option>'; elCidade.disabled = true; } return; } if(elCidade) { elCidade.innerHTML = '<option>...</option>'; elCidade.disabled = true; fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${uf}/municipios`).then(r=>r.json()).then(c=>{ elCidade.innerHTML = '<option>Selecione</option>'; c.sort((a,b)=>a.nome.localeCompare(b.nome)); c.forEach(i=>{ const o=document.createElement('option'); o.value=i.nome.toUpperCase(); o.text=i.nome.toUpperCase(); elCidade.appendChild(o); }); elCidade.disabled=false; }); } }
window.carregarListaHorarios = function() { const select = document.getElementById('hora'); if (!select) return; select.innerHTML = '<option value="">Selecione...</option>'; const horarios = [ "08:00", "08:15", "08:30", "08:45", "09:00", "09:15", "09:30", "09:45", "10:00", "10:15", "10:30", "10:45", "11:00", "11:15", "11:30", "11:45", "12:00", "12:15", "12:30", "12:45", "13:00", "13:15", "13:30", "13:45", "14:00", "14:15", "14:30", "14:45", "15:00", "15:15", "15:30", "15:45", "16:00", "16:15", "16:30", "16:45", "17:00", "17:15", "17:30" ]; horarios.forEach(hora => { const option = document.createElement('option'); option.value = hora; option.textContent = hora; select.appendChild(option); }); }
...
window.renderizarTabela = function(lista) {
    ...
    lista.forEach(item => {
        ...
        let servicosExtras = []; if (item.tanato === 'SIM') servicosExtras.push('TANATOPRAXIA'); if (item.invol === 'SIM') servicosExtras.push('INVOL'); if (item.translado === 'SIM') servicosExtras.push('TRANSLADO'); if (item.urna_opc === 'SIM') servicosExtras.push('URNA'); if (servicosExtras.length > 0) displayResponsavel += `<br><span style="font-size:10px; font-weight:bold;">SERVIÇOS: ${servicosExtras.join(', ')}</span>`; 
        ...
        if (item.data_obito && item.data_ficha) {
            ...
            if (item.hora_obito && item.hora) { const ini = new Date(`${item.data_obito}T${item.hora_obito}`), fim = new Date(`${item.data_ficha}T${item.hora}`); if (!isNaN(ini) && !isNaN(fim)) { const dMs = fim - ini; txtT = `<br><span style="font-weight:bold; font-size:10px;">TEMPO DE FALECIMENTO: ${Math.floor(dMs / 3600000)}H ${Math.round(((dMs % 3600000) / 60000))}M</span>`; } }
...
window.renderizarTabelaAgencia = function(lista) {
    ...
    const renderChip = (key, label, item) => { const url = item[`url_${key}`]; if (item[`agencia_chk_${key}`]) { if (url) return `<a href="${url}" target="_blank" onclick="event.stopPropagation();" style="text-decoration:none;"><span class="doc-chip tem" style="cursor:pointer;" title="Ver Anexo">📎 ${label}</span></a>`; return `<span class="doc-chip tem">${label}</span>`; } return `<span class="doc-chip">${label}</span>`; };
    lista.forEach(item => {
...
window.assumirProcessoAgencia = function(id, isTransfer = false) {
    ...
    if (confirm(isTransfer ? `Deseja transferir a responsabilidade deste processo para você?` : `Deseja assumir a responsabilidade por este processo na Agência?`)) { getDB().collection("atendimentos").doc(id).update({ agencia_atendente: usuarioLogado.nome }).then(() => { getDB().collection("auditoria").add({ data_log: new Date().toISOString(), usuario: usuarioLogado.nome, acao: isTransfer ? "TRANSFERIU RESPONSABILIDADE (AGÊNCIA)" : "ASSUMIU AGÊNCIA", detalhe: `ID: ${id}` }); }).catch(e => { alert("Erro ao assumir atendimento."); }); }
}
...
window.confirmarTransferenciaResponsavel = function() {
    ...
    getDB().collection("atendimentos").doc(window.idTransferenciaResponsavelAtual).update({ agencia_atendente: novoResponsavel }).then(() => { getDB().collection("auditoria").add({ data_log: new Date().toISOString(), usuario: usuarioLogado ? usuarioLogado.nome : 'Anon', acao: "REPASSOU PROCESSO (AGÊNCIA)", detalhe: `ID: ${window.idTransferenciaResponsavelAtual} | Para: ${novoResponsavel}` }); alert("Processo repassado com sucesso!"); const el = document.getElementById('agencia_atendente_modal'); if (el && document.getElementById('modal-agencia').style.display === 'block') el.innerText = novoResponsavel.toUpperCase(); window.fecharModalTransferirResponsavel(); }).catch(e => { alert("Erro ao transferir processo."); });
}
window.abrirModalOneDrive = function() { 
    const el = document.getElementById('onedrive_nome_pasta'); if(el) el.innerText = window.nomePastaPadrao || 'PROTOCOLO_NOME'; 
    const mo = document.getElementById('modal-onedrive'); if(mo) { mo.style.display = 'flex'; mo.style.zIndex = '10000'; } 
...
window.salvarRootOneDrive = function() { const link = document.getElementById('input_root_onedrive').value; localStorage.setItem('onedrive_root', link); document.getElementById('iframe_onedrive').src = link || 'https://onedrive.live.com/'; document.getElementById('btn_nova_aba_onedrive').href = link || 'https://onedrive.live.com/'; alert('Pasta raiz salva com sucesso!'); }
window.fecharModalOneDrive = function() { safeDisplay('modal-onedrive', 'none'); }
window.copiarNomePasta = function() { const texto = document.getElementById('onedrive_nome_pasta').innerText; navigator.clipboard.writeText(texto).then(() => { alert("Nome copiado! Vá para o OneDrive, crie a pasta e cole este nome."); }).catch(err => { alert("Erro ao copiar."); }); }
...
window.realizarBusca = function() {
    ...
    unsubscribe = getDB().collection("atendimentos").orderBy("nome").startAt(termo).endAt(termo + "\uf8ff").limit(20).onSnapshot((snap) => { let lista = []; snap.forEach(doc => { let d = doc.data(); d.id = doc.id; lista.push(d); }); dashboardAtual === 'acolhimento' ? window.renderizarTabela(lista) : window.renderizarTabelaAgencia(lista); });
}
window.atualizarListener = function(data, local) {
    if(!getDB()) return; if (unsubscribe) unsubscribe();
    unsubscribe = getDB().collection("atendimentos").where("data_ficha", "==", data).onSnapshot((snap) => { let lista = []; snap.forEach(doc => { let d = doc.data(); d.id = doc.id; if ((d.local || "CEMITÉRIO DO MARUÍ") === local) lista.push(d); }); lista.sort((a,b) => (a.hora < b.hora ? -1 : 1)); dashboardAtual === 'acolhimento' ? window.renderizarTabela(lista) : window.renderizarTabelaAgencia(lista); });
}
window.liberarAcesso = function() { safeDisplay('tela-bloqueio', 'none'); sessionStorage.setItem('usuarioLogado', JSON.stringify(usuarioLogado)); const il = document.getElementById('filtro-local'); if(il) { window.atualizarListener(window.pegarDataAtualLocal(), il.value); window.atualizarLabelQuadra(il.value); } }
...
window.carregarEstatisticas = function(modo) {
    ...
    database.collection("atendimentos").where("data_ficha", ">=", dString).onSnapshot(snap => {
        ...
        let quadras = {}, hospitais = {}, bairros = {}, municipios = {}, valores = {"Até R$ 500":0, "R$ 501 a R$ 1000":0, "R$ 1001 a R$ 2000":0, "Acima de R$ 2000":0}; let totalValores = 0, qtdValores = 0;
        snap.forEach(doc => {
            ...
            if(d.data_hora_atendimento && d.data_ficha && d.hora) { const start = new Date(d.data_hora_atendimento), end = new Date(d.data_ficha + 'T' + d.hora); if(!isNaN(start) && !isNaN(end)) { const diffHrs = (end - start) / 3600000; if(diffHrs < 12) tempos["Menos de 12h"]++; else if(diffHrs <= 24) tempos["12h a 24h"]++; else if(diffHrs <= 48) tempos["24h a 48h"]++; else tempos["Mais de 48h"]++; } }
        });
        const draw = (id, dataObj, lbl, type='bar', color='#3b82f6') => {
            const ctx = document.getElementById(id); if(!ctx || !window.Chart) return; const sorted = Object.entries(dataObj).sort((a,b) => b[1] - a[1]).slice(0, 10);
...
window.buscarContribuintes = function() {
    ...
    getDB().collection("atendimentos").get().then(snap => {
        ...
        results.forEach(c => { let enderecoCompleto = c.endereco ? `${c.endereco}, ${c.numero} - ${c.bairro}` : 'Não informado'; ul.innerHTML += `<li class="table-equipe-row"><div style="flex: 2; font-weight: 600; color: #1e293b;">${c.nome}</div><div style="flex: 1.5; color: #475569; font-size: 13px;">${c.cpf} <br> <span style="font-size: 11px; color: #94a3b8;">RG: ${c.rg || '-'}</span></div><div style="flex: 1.5; color: #475569; font-size: 13px;">${c.telefone}</div><div style="flex: 2; color: #475569; font-size: 12px; line-height: 1.2;">${enderecoCompleto}</div><div style="width: 60px; display: flex; justify-content: flex-end;"><button class="btn-action-edit" onclick="editarContribuinte('${c.cpf}', '${c.nome}')" title="Editar Contribuinte">✏️</button></div></li>`; });
    });
}
...
window.baixarRelatorioCompleto = function() {
    if(!getDB()) return; if(!confirm("Baixar relatório?")) return; if(typeof XLSX === 'undefined') { alert("Biblioteca Excel não carregada."); return; }
    getDB().collection("atendimentos").get().then(snap => {
...
window.baixarLogsExcel = function() { if(typeof XLSX === 'undefined') { alert("Erro: Biblioteca Excel ausente."); return; } getDB().collection("atendimentos").limit(100).orderBy("data_ficha", "desc").get().then(snap => { let dados = []; snap.forEach(doc => { const d = doc.data(); const dataF = d.data_ficha ? d.data_ficha.split('-').reverse().join('/') : '-'; const atendente = d.atendente_sistema ? d.atendente_sistema.toUpperCase() : 'SISTEMA'; dados.push({ "Data": dataF, "Usuário": atendente, "Ação/Detalhes": `Cadastro: ${d.nome}` }); }); const ws = XLSX.utils.json_to_sheet(dados); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Auditoria"); XLSX.writeFile(wb, "Logs_Auditoria.xlsx"); }); }
window.baixarLogsPDF = function() { if(!window.jspdf) { alert("Erro: Biblioteca PDF ausente."); return; } const { jsPDF } = window.jspdf; const doc = new jsPDF(); getDB().collection("atendimentos").limit(100).orderBy("data_ficha", "desc").get().then(snap => { let body = []; snap.forEach(doc => { const d = doc.data(); const dataF = d.data_ficha ? d.data_ficha.split('-').reverse().join('/') : '-'; const atendente = d.atendente_sistema ? d.atendente_sistema.toUpperCase() : 'SISTEMA'; body.push([dataF, atendente, `Cadastro: ${d.nome}`]); }); doc.text("Relatório de Auditoria", 14, 10); doc.autoTable({ head: [['Data', 'Usuário', 'Ação/Detalhes']], body: body, startY: 20 }); doc.save("Logs_Auditoria.pdf"); }); }
...
window.baixarTodosPDF = function() {
    if(!window.jspdf) { alert("Biblioteca PDF não carregada."); return; } const { jsPDF } = window.jspdf; const doc = new jsPDF('landscape');
    getDB().collection("atendimentos").get().then(snap => {
...
window.gerarBackup = async function() {
    if(!getDB()) return;
    try {
        const btn = event.target; const originalText = btn.innerText; btn.innerText = "⏳ Gerando..."; btn.disabled = true; let backupData = { atendimentos: [], equipe: [], auditoria: [] };
        ...
        const blob = new Blob([JSON.stringify(backupData)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; const date = new Date().toISOString().slice(0,10); a.download = `backup_funeraria_${date}.json`;
...
window.restaurarBackup = function() {
    const fileInput = document.getElementById('file-restore'); const file = fileInput.files[0]; if (!file) { alert("Por favor, selecione um arquivo de backup (.json)."); return; }
...
window.editarFuncionario = function(id) { getDB().collection("equipe").doc(id).get().then(doc => { if(doc.exists) { const u = doc.data(); document.getElementById('edit-id').value = doc.id; document.getElementById('edit-nome').value = u.nome; document.getElementById('edit-login').value = u.login; document.getElementById('edit-email').value = u.email; document.getElementById('edit-senha').value = u.senha; document.getElementById('box-novo-usuario').classList.add('hidden'); document.getElementById('div-editar-usuario').classList.remove('hidden'); } }); }
window.salvarEdicaoUsuario = function() { const id = document.getElementById('edit-id').value, nome = document.getElementById('edit-nome').value, email = document.getElementById('edit-email').value, senha = document.getElementById('edit-senha').value; if(!nome || !senha) { alert("Nome e senha são obrigatórios."); return; } getDB().collection("equipe").doc(id).update({ nome, email, senha }).then(() => { alert("Usuário atualizado!"); window.cancelarEdicao(); }).catch(e => alert("Erro: " + e)); }
...
window.editar = function(id) {
    getDB().collection("atendimentos").doc(id).get().then(doc => {
        if(doc.exists) {
            const d = doc.data(); for (let key in d) { let targetId = key === 'data_ficha' ? 'data_ficha_modal' : key; const el = document.getElementById(targetId); if(el) el.value = d[key]; }
...
window.mesclarEBaixarPDFsAcolhimento = async function(event) {
    ...
    try {
        const btn = event.target; const oldTxt = btn.innerText; btn.innerText = "Mesclando..."; btn.disabled = true; const pdfDoc = await window.PDFLib.PDFDocument.create();
...
// --- IMPRESSÕES (COMPACTADAS PARA NÃO CORTAR O JS) ---
...
window.gerarAutorizacao = function() {
    ...
    let txtSep = (d.tipo_sepultura || "").toUpperCase(); const co = d.classificacao_obito || "ADULTO"; let c = co === "ANJO" ? "ANJO" : "ADULTO"; let cond = "ALUGUEL (3 ANOS)"; if (txtSep.includes("PERPETU")) { txtSep += " - " + c; cond = `PERPÉTUA (L: ${d.livro_perpetua||'-'} / F: ${d.folha_perpetua||'-'})`; } else if (txtSep.includes("MEMBRO")) { txtSep = `MEMBRO AMPUTADO`; cond = "N/A"; } else { txtSep += " - " + c; }
...
window.gerarComprovante = function() {
    if (!dadosAtendimentoAtual) return; const d = dadosAtendimentoAtual; const chk = (cond) => cond ? '(X)' : '( )'; const fd = (dataStr) => dataStr ? dataStr.split('-').reverse().join('/') : ''; const p = d.protocolo || ""; let dhT = ""; if (d.data_hora_atendimento) { const parts = d.data_hora_atendimento.split('T'); if (parts.length === 2) { const dp = parts[0].split('-'); dhT = `${dp[2]}/${dp[1]}/${dp[0]} AS ${parts[1]}`; } } else { const now = new Date(); dhT = `${now.getDate().toString().padStart(2,'0')}/${(now.getMonth()+1).toString().padStart(2,'0')}/${now.getFullYear()} AS ${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}`; }
    const im = (d.local && d.local.includes("MARUÍ")), is = (d.local && d.local.includes("SÃO FRANCISCO")), ii = (d.local && d.local.includes("ITAIPU")), cc = (d.cap && !d.cap.toUpperCase().includes("SEM")); let tempD = ""; if (d.data_obito && d.hora_obito && d.hora && d.data_ficha) { const diff = new Date(d.data_ficha+'T'+d.hora) - new Date(d.data_obito+'T'+d.hora_obito); if (diff > 0) tempD = `${Math.floor(diff/3600000)}h ${Math.round(((diff%3600000)/60000))}min`; }
    const chkEC = (val) => (d.estado_civil||"") === val ? '(X)' : '( )'; const rel = d.parentesco ? `(${d.parentesco})` : ''; let txtSep = (d.tipo_sepultura || "").toUpperCase(), co = d.classificacao_obito || "ADULTO", txtHO = d.hora_obito || ""; if (d.ignorar_hora_obito === 'SIM') txtHO += " (IGNORADO)"; let c = co === "ANJO" ? "ANJO" : "ADULTO";
    ...
    let bFam = assinaturaResponsavelImg ? `<div style="text-align:center;height:45px;"><img src="${assinaturaResponsavelImg}" style="max-height:40px;max-width:80%;"></div>` : `<div style="height:45px;"></div>`; let bAte = assinaturaAtendenteImg ? `<div style="text-align:center;height:45px;"><img src="${assinaturaAtendenteImg}" style="max-height:40px;max-width:80%;"></div>` : `<div style="height:45px;"></div>`; let nAte = (d.atendente_sistema || (usuarioLogado ? usuarioLogado.nome : 'N/A')).toUpperCase();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => { if(getDB()) { getDB().collection("atendimentos").orderBy("data_hora_atendimento", "desc").limit(50).get().then(snap => { snap.forEach(doc => { const d = doc.data(); if(d.data_ficha_modal && !d.data_ficha) { getDB().collection("atendimentos").doc(doc.id).update({ data_ficha: d.data_ficha_modal }); } }); }); } }, 3000);
});
...
window.visualizar = function(id) {
    ...
    getDB().collection("atendimentos").doc(id).get().then(doc => {
        if(doc.exists) {
            ...
            const m = { 'view_protocolo': d.protocolo || 'S/ PROTOCOLO', 'view_hora': d.hora || '--:--', 'view_nome': (d.nome || 'NÃO INFORMADO').toUpperCase(), 'view_causa': (d.causa || 'NÃO INFORMADA').toUpperCase(), 'view_resp_completo': (d.resp_nome || 'N/I').toUpperCase() + (d.parentesco ? ` (${d.parentesco})` : ''), 'view_resp_cpf': d.resp_cpf || '-', 'view_resp_rg': d.resp_rg || '-', 'view_telefone': d.telefone || '-', 'view_funeraria': (d.funeraria || 'N/I').toUpperCase(), 'view_atendente': (d.atendente_sistema || 'N/I').toUpperCase(), 'view_combo_urna': d.combo_urna || '-', 'view_hospital_completo': (d.hospital || 'N/I').toUpperCase(), 'view_cap': (d.cap || 'N/I').toUpperCase(), 'view_urna_info': d.urna_info || 'Sem observações adicionais.', 'view_isencao_completa': isenStr, 'view_servicos_adicionais': servs.length > 0 ? servs.join(', ') : 'NENHUM', 'view_data_sepultamento': d.data_ficha ? d.data_ficha.split('-').reverse().join('/') : '--/--/----', 'view_tipo_urna_detalhe': (d.tipo_urna_detalhe || '').toUpperCase() };
            ...
            if (mapContainer && mapFrame && mapLink) { 
                ...
                if (cleanCoords && cleanCoords.includes(',')) { mapContainer.style.display = 'block'; mapFrame.innerHTML = `<iframe width="100%" height="100%" frameborder="0" style="border:0" src="https://maps.google.com/maps?q=...${cleanCoords}&hl=pt-BR&z=16&output=embed"></iframe>`; mapLink.href = `https://maps.google.com/maps?q=...${cleanCoords}`; } 
...
window.enviarWppTemplate = function(tipo) {
    ...
    if (tipo === 'gps') {
        const c = d.geo_coords ? d.geo_coords.replace(/[^0-9.,\-]/g, '') : ''; if (!t) { alert("Sem telefone cadastrado."); return; } if (!c) { alert("Sem GPS cadastrado."); return; }
        texto = `Localização da Sepultura: https://maps.google.com/maps?q=...${c}`;
    } else if (tipo === 'info') {
        if (!t) { alert("Sem telefone cadastrado."); return; } let dataExumacao = ""; 
...
window.enviarSMS = function() { 
    ...
    if (!t) { alert("Sem telefone."); return; } window.location.href = `sms:55${t}?body=${encodeURIComponent('Localização da Sepultura: https://maps.google.com/maps?q=...$' + c)}`; 
}
...
window.excluir = function(id) { if(confirm('Tem certeza que deseja excluir?')) getDB().collection("atendimentos").doc(id).delete(); };
