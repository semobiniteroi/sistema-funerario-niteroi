...
function inicializarSistema() {
    ...
    // CORREÇÃO DE ACESSO: Valida cache vs DB
...
window.copiarTexto = function(id) { 
    ...
    navigator.clipboard.writeText(texto)
        .then(() => alert("Nome copiado com sucesso!"))
        .catch(e => alert("Erro ao copiar o texto. Selecione e copie manualmente.")); 
}
...
window.salvarAssinatura = function() {
    ...
    if (canvas && dadosAtendimentoAtual && dadosAtendimentoAtual.id) { 
        ...
        getDB().collection("atendimentos").doc(dadosAtendimentoAtual.id).update(updateData)
            .then(() => window.fecharModalAssinatura()); 
    }
}
...
window.confirmarTransferencia = function() {
    ...
    if(confirm(`Confirmar a transferência de ${localAntigo} para ${novoLocal}?`)) { 
        getDB().collection("atendimentos").doc(dadosAtendimentoAtual.id).update({ local: novoLocal })
            .then(() => { 
...
window.renderizarTabela = function(lista) {
    ...
    lista.forEach(item => {
        ...
        // Se for PARTICULAR, aplica layout especial
...
window.renderizarTabelaAgencia = function(lista) {
    ...
    lista.forEach(item => {
        ...
        // RENDERIZAÇÃO DO CARD PARTICULAR
        if (item.tipo_registro === 'PARTICULAR') {
            ...
            card.innerHTML = `
                ...
                <div class="agencia-card-footer" style="background: #f3e8ff; border-top: 1px solid #e9d5ff;">
                     <button class="btn-novo" style="background:#ef4444; color:white; padding: 6px 12px; font-size: 12px; width:100%;" onclick="event.stopPropagation(); window.excluir('${item.id}')">🗑️ Remover Registro</button>
                </div>
            `;
        ...
        // RENDERIZAÇÃO DO CARD MUNICIPAL (Padrão)
        ...
        let statusDocsAcolhimento = item.url_docs_acolhimento 
            ? `<span class="badge-status badge-sucesso" style="cursor:pointer;" onclick="event.stopPropagation(); window.open('${item.url_docs_acolhimento}', '_blank')" title="Acessar Documentos do Acolhimento">OK (NUVEM)</span>` 
            : `<span class="badge-status badge-pendente" title="Acolhimento ainda não anexou os documentos">PENDENTE</span>`;
...
window.assumirProcessoAgencia = function(id, isTransfer = false) {
    ...
    if (confirm(isTransfer ? `Deseja transferir a responsabilidade deste processo para você?` : `Deseja assumir a responsabilidade por este processo na Agência?`)) {
        getDB().collection("atendimentos").doc(id).update({ agencia_atendente: usuarioLogado.nome })
            .then(() => { 
                getDB().collection("auditoria").add({ 
                    data_log: new Date().toISOString(), 
...
window.confirmarTransferenciaResponsavel = function() {
    ...
    getDB().collection("atendimentos").doc(window.idTransferenciaResponsavelAtual).update({ agencia_atendente: novoResponsavel })
        .then(() => {
            getDB().collection("auditoria").add({ 
                data_log: new Date().toISOString(), 
                usuario: usuarioLogado ? usuarioLogado.nome : 'Anon', 
    ...
    database.collection("atendimentos").where("data_ficha", ">=", dString).onSnapshot(snap => {
        ...
        const draw = (id, dataObj, lbl, type='bar', color='#3b82f6') => {
            ...
            chartInstances[id] = new Chart(ctx, { 
                ...
                options: { indexAxis: type==='bar'?'y':'x', maintainAspectRatio: false } 
            });
            ...
            if (cpf.includes(termo.replace(/\D/g, '')) || nome.includes(termo) || rg.includes(termo) || tel.includes(termo)) {
                ...
                if (key && !contribuintesMap[key]) { 
                    contribuintesMap[key] = { 
                        id: doc.id, cpf: d.resp_cpf || '', nome: d.resp_nome || '', rg: d.resp_rg || '', 
                        telefone: d.telefone || '', endereco: d.resp_endereco || '', numero: d.resp_numero || '', 
                        bairro: d.resp_bairro || '', cidade: d.resp_cidade || '', uf: d.resp_uf || '', 
                        cep: d.resp_cep || '', complemento: d.resp_complemento || '' 
                    }; 
                }
            }
        });
        ...
        results.forEach(c => {
            ...
            li.innerHTML = `
                ...
                <div style="width: 60px; display: flex; justify-content: flex-end;">
                    <button class="btn-action-edit" onclick="editarContribuinte('${c.cpf}', '${c.nome}')" title="Editar Contribuinte">✏️</button>
                </div>`;
...
window.editarContribuinte = function(cpf, nome) {
    ...
    if (cpf) query = query.where("resp_cpf", "==", cpf); 
    else query = query.where("resp_nome", "==", nome);
    ...
    getDB().collection("equipe").onSnapshot(snap => { 
        ...
        snap.forEach(doc => { 
            ...
            li.innerHTML = `
                ...
                <div class="col-pass">*** <button class="btn-icon" style="background:#f8fafc; padding:6px; border-radius:50%; border:none; cursor:pointer;" onclick="alert('Senha: ${u.senha}')">👁️</button></div>
                <div class="col-actions">
                    <button class="btn-action-edit" onclick="window.editarFuncionario('${doc.id}')">✏️</button>
                    <button class="btn-action-delete" onclick="window.excluirFuncionario('${doc.id}')">🗑️</button>
                </div>`; 
            fragment.appendChild(li);
        }); 
...
// ============================================================================
// TEMPLATES HTML DE IMPRESSÃO E RESUMO
// ============================================================================
...
window.gerarAutorizacao = function(){
    ...
    <span>${checkForm(d.tipo_sepultura,'CRJ')} Cova Rasa Anjo</span> 
    <span>${checkForm(d.tipo_sepultura,'GA')} GAVETA ADULTO</span> 
    <span>${checkForm(d.tipo_sepultura,'GJ')} GAVETA ANJO</span> 
    <span>${checkForm(d.tipo_sepultura,'CA')} CARNEIRA ADULTO</span> 
    <span>${checkForm(d.tipo_sepultura,'CJ')} CARNEIRA ANJO</span> 
    <span>Nº Sepultura <span class="ul-texto" style="width:8%;">${d.sepul||''}</span></span> 
    <span>Qd. <span class="ul-texto" style="width:8%;">${d.qd||''}</span></span> 
    <span>Rua <span class="ul-texto" style="width:8%;"></span></span>
    </div>
...
// ============================================================================
// VISUALIZAR FICHA DE RESUMO (OLHINHO)
// ============================================================================
...
window.enviarWppTemplate = function(tipo) {
    ...
    // MAPEAMENTO DE ENDEREÇOS E LINKS DOS CEMITÉRIOS (CORRIGIDO PARA MAPS.GOOGLE.COM)
    let enderecoStr = "";
    let mapaStr = "";
    if (d.local) {
        if (d.local.includes('MARUÍ')) {
            enderecoStr = "Rua General Castrioto, 414 - Barreto, Niterói - RJ";
            mapaStr = "https://maps.google.com/?q=Cemitério+do+Maruí,+Niterói";
        } else if (d.local.includes('FRANCISCO')) {
            enderecoStr = "Rua Tapajós, 75 - São Francisco, Niterói - RJ";
            mapaStr = "https://maps.google.com/?q=Cemitério+de+São+Francisco,+Niterói";
        } else if (d.local.includes('ITAIPU')) {
            enderecoStr = "Rua Paulina Rabello, 78 - Itaipu, Niterói - RJ";
            mapaStr = "https://maps.google.com/?q=Cemitério+de+Itaipu,+Niterói";
        }
    }
